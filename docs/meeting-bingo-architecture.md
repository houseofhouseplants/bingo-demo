...
graph TD
    subgraph Browser["Browser Environment"]
        ...
        subgraph BAPI["Browser APIs (Built-in, Free)"]
            WSA["Web Speech API"]
            AC["Audio Context"]
            CA["Clipboard API"]
            SA["Share API"]
...
```mermaid
flowchart LR
    subgraph flow1["1. Card Generation Flow"]
        A1[Select Category] --> B1[Shuffle Words] --> C1[Pick 24 Words] --> D1[Render Card]
    end

    subgraph flow2["2. Speech Recognition Flow"]
        A2[Audio Input] --> B2[Speech API] --> C2[Transcript Text] --> D2[Word Detection]
        D2 --> E2{Match Found?}
        E2 -->|Yes| F2[Fill Square] --> G2[Update UI]
    end

    subgraph flow3["3. Win Detection Flow"]
        A3[Square Filled] --> B3[Check Lines] --> C3{BINGO?}
        C3 -->|Yes| D3[Celebrate & Share]
    end
```
...
```mermaid
graph TD
    MP["Future: Multiplayer"] --> A["**Option A: WebSocket Server**\nRequires Backend\n• Real-time sync across players\n• Shared game state\n• Live leaderboard\n• Cost: $5–20/month"]
    MP --> B["**Option B: Peer-to-Peer**\nWebRTC — No backend required\n• Direct browser connections\n• Limited to ~10 players\n• Cost: Free"]
    MP --> C["**Option C: Firebase Realtime DB**\nManaged real-time sync\n• Free tier generous\n• Easy implementation\n• Cost: Free up to 100 concurrent"]
```
